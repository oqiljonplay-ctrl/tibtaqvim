...
const SOURCE_LABEL: Record<string, string> = { kanal: "Kanal", guruh: "Guruh" };
...
export default function SuperPromotionsPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/clinics", { credentials: "include" })
      .then((r) => r.json())
      .then((d) => {
        const list: Clinic[] = (d.data ?? []).map((c: { id: string; name: string }) => ({ id: c.id, name: c.name }));
        setClinics(list);
  ...
  const loadPromotions = useCallback((clinicId: string) => {
    if (!clinicId) return;
    setLoading(true);
    fetch(`/api/admin/promotions?clinicId=${clinicId}`, { credentials: "include" })
      .then((r) => r.json())
  ...
  useEffect(() => {
    if (selectedClinic) loadPromotions(selectedClinic);
  }, [selectedClinic, loadPromotions]);

  function openAdd() {
    setForm({ ...EMPTY_FORM, clinicId: selectedClinic });
    setEditingId(null);
    setShowForm(true);
    setError(null);
  }
    ...
    <Container size="lg">
      <Stack gap={6}>
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            <h1 className="text-2xl font-bold text-gray-900">📣 Telegram postlar</h1>
            <p className="text-gray-500 text-sm mt-0.5">Klinikalar Telegram widget dropdown uchun postlar</p>
          </div>
          <button
            onClick={openAdd}
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <label className="text-sm font-medium text-gray-700">Klinika:</label>
          <select
            value={selectedClinic}
            onChange={(e) => setSelectedClinic(e.target.value)}
            className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm bg-white min-w-[200px]"
          >
            {clinics.map((c) => (
        ...
        {showForm && (
          <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6">
            <h2 className="text-lg font-bold mb-4">{editingId ? "Postni tahrirlash" : "Yangi post qo'shish"}</h2>
            {error && (
            ...
            <Stack gap={4}>
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Tartib (sortOrder)</label>
                  <input
                    type="number"
                    className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-10 text-center">
            ...
            <p className="text-gray-500 text-sm">Hozircha post yo'q. "Yangi post" tugmasini bosing.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-sm">
              <thead className="bg-gray-50 border-b border-gray-100">
                <tr>
                  ...
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide hidden md:table-cell">Manba</th>
                  <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wide hidden lg:table-cell">Sana</th>
              ...
              <tbody className="divide-y divide-gray-50">
                {promotions.map((p) => (
                  <tr key={p.id} className="hover:bg-gray-50 transition">
                    ...
                    <td className="px-4 py-3 text-gray-500 hidden md:table-cell">{SOURCE_LABEL[p.source]}</td>
